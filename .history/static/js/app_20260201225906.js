...
class CyberSRSApp {
    ...
    setupEventListeners() {
        ...
        // Keyboard shortcuts - use event delegation for robustness
        document.addEventListener('keydown', (e) => {
            if (e.key === 'Enter') {
                e.preventDefault();
                // Check if we have a current word and the submit button is not disabled
                if (window.app.currentWord && !document.getElementById('submit-btn').disabled) {
    ...
    handleKeyboard(e) {
        // Only handle Enter key when pressed in the answer input or anywhere on the page
        if (e.key === 'Enter') {
            e.preventDefault();
            // Check if we have a current word and the submit button is not disabled
            if (this.currentWord && !document.getElementById('submit-btn').disabled) {
                this.submitAnswer();
            }
        }
    }
