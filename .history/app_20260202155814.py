...
def session_start():
    try:
        ...
        if not session_token or not start_time:
            return jsonify({'error': 'token and start_time required'}), 400
        ...
        # Insert session
        cursor.execute('''
            INSERT INTO learning_sessions (session_token, user_ip, user_agent, start_time)
            VALUES (?, ?, ?, ?)
        ''', (session_token, request.remote_addr, request.headers.get('User-Agent'), start_time))
...
def session_complete():
    try:
        ...
        if not session_token or not end_time:
            return jsonify({'error': 'token and end_time required'}), 400
        ...
        cursor.execute('''
            ...
            SET end_time = ?, total_questions = ?, correct_answers = ?, accuracy_rate = ?, completed = 1
            WHERE session_token = ?
        ''', (end_time, total_questions, correct_answers, accuracy_rate, session_token))

        # Insert answers
        for answer in answers:
            cursor.execute('''
                INSERT INTO user_answers (session_token, word_id, user_answer, correct, response_time)
                VALUES (?, ?, ?, ?, ?)
            ''', (session_token, answer.get('wordId'), answer.get('userAnswer'),
                  answer.get('isCorrect'), answer.get('responseTime')))
...
def session_start():
    ...
    try:
        cursor.execute('''
            INSERT INTO learning_sessions 
            (session_token, user_agent, start_time) 
            VALUES (?, ?, ?)
        ''', (data['session_token'], data['user_agent'], data['start_time']))
        conn.commit()
        return jsonify({"status": "ok"})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def session_answer():
    ...
    try:
        cursor.execute('''
            ...
            (session_token, word_id, user_answer, correct, response_time, answered_at) 
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (
            data['session_token'], data['word_id'], data['user_answer'],
            data['correct'], data['response_time'], data['answered_at']
        ))
        conn.commit()
        return jsonify({"status": "ok"})
    except Exception as e:
...
def session_complete():
    ...
    try:
        cursor.execute('''
            UPDATE learning_sessions 
            SET end_time = ?, total_questions = ?, correct_answers = ?, accuracy_rate = ?
            WHERE session_token = ?
        ''', (
            data['end_time'], data['total_questions'], 
            data['correct_answers'], data['accuracy_rate'],
            data['session_token']
        ))
        conn.commit()
        return jsonify({"status": "ok"})
