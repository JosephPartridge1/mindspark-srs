...
def session_answer():
    ...
    # Validate required fields
    required = ['session_token', 'word_id', 'user_answer', 'correct']
    for field in required:
        if field not in data:
            return jsonify({"error": f"Missing field: {field}"}), 400
    ...
    try:
        ...
        cursor.execute('SELECT id FROM learning_sessions WHERE session_token = ?', (data['session_token'],))
        session_exists = cursor.fetchone()

        if not session_exists:
            return jsonify({"error": f"Session token {data['session_token']} does not exist"}), 400

        # Insert answer without answered_at (let database use DEFAULT CURRENT_TIMESTAMP)
        cursor.execute('''
        ...
        ''', (
            ...
            data['user_answer'],
            data['correct'],
            data.get('response_time')  # Use get() in case it's missing
        ))

        conn.commit()
        return jsonify({"status": "answer_saved", "answer_id": cursor.lastrowid})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
    finally:
        conn.close()
